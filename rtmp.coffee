...
class RTMPClient extends RTMPProtocol
  ...
  send: (cmd, cb) ->
    ...
    timeoutId = setTimeout closeSocket, 5000
  ...
  ###
    console.log 'gggggggggggg', message.streamId, @nextChannelId
    dataQ = Buffer.concat [dataQ, data]
    if dataQ.length >1000 and @nextChannelId > 6 and @socket.writable
      @socket.write(dataQ)
      dataQ = new Buffer(0)
    else if @nextChannelId <= 6 and @socket.writable
      @socket.write(dataQ)
      dataQ = new Buffer(0)
  ###
...
#        @socket.destroySoon()
      ...
      console.log 'UNKNOWN MSG?', msg if @debug
...
class RTMPCommand
  ...
  @fromMessage: (message) ->
    ...
    if cmd.name is '_error'
      console.log cmd.args[0].object.rootCause
      if cmd.args[0]?.object?.rootCause?.object?.errorCode is 'LOGIN-0001'
        fs.writeFile('ver', cmd.args[0].object.rootCause.object.substitutionArguments[1])
      else
        logFile 'ERROR_'+cmd.args[0].object.rootCause.object.errorCode+'.txt', cmd
      console.log cmd
      console.log hexy.hexy(data)
